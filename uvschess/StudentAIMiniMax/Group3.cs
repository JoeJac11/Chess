    ...
    public class StudentAIMinimax : IChessAI
    {
        ...
        {   // the order as listed is in the shape of the board as we look at it
            ...
            [ChessPiece.BlackPawn] = new List<List<int>>()
            {
                ...
                new List<int>() { 100, 100, 100, 100, 100, 100, 100, 100 }
            },
        ...
        public int evaluateBoard(ChessMove m, ChessBoard board, ChessColor myColor)
        {
            ...
            int sum = 0;
            ChessPiece from = board[m.From];
            sum += positionVals[board[m.From]][m.To.X][m.To.Y];
        ...
        public ChessMove GetNextMove(ChessBoard board, ChessColor myColor)
        {
            ...
            if (chosenMove == null)
            {
                foreach (ChessMove move in validMoves)
                {
                    ...
                    if (evaluateBoard(move, board, myColor) > 0)
                    {
                        return move;
                    }
                }
                Random rand = new Random(DateTime.Now.Millisecond);
                return validMoves[rand.Next(0, validMoves.Count)];
            }
            return chosenMove;
        }
